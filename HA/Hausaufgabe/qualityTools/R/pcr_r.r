...
pcr = function(x, distribution = "normal", lsl, usl, target, boxcox = FALSE, lambda = c(-5, 5), main, 
    xlim, ylim, grouping = NULL, std.dev = NULL, conf.level = 0.9973002, start, lineWidth = 1, lineCol = "red", 
    lineType = "solid", specCol = "red3", specWidth = 1, cex.text = 2, cex.val = 1.5, cex.col = "darkgray", 
    plot = TRUE, bounds.lty = 3, bounds.col = "red", ...) {
    DB = FALSE
    ...
    if (any(x < 0) && any(distribution == c("beta", "chi-squared", "exponential", "f", "geometric", 
        "lognormal", "log-normal", "negative binomial", "poisson", "weibull", "gamma"))) 
    ...
    if (plot == TRUE) {
        ...
        if (not3distr) {
            ...
            for (i in 3:(3 + length(estimates) - 1)) {
                ...
                try(text(2, rev(index)[i + 1], paste("=", format(estimates[[j]], digits = 3)), pos = 4, 
                  cex = cex.val), silent = TRUE)
                j = j + 1
            }
        }
        if (any3distr) {
            ...
            for (i in 3:(3 + length(estimates) - 1)) {
                ...
                try(text(2.5, rev(index)[i + 1], paste("=", format(estimates[[j]], digits = 3)), pos = 4, 
                  cex = cex.val), silent = TRUE)
                j = j + 1
            }
        }
        qqPlot(x[, 1], y = distribution, ylab = "", main = "", axes = FALSE, bounds.lty = bounds.lty, 
        ...
        else invisible(list(lambda = lambda, cp = cp, cpk = cpk, cpl = cpl, cpu = cpu, ppt = ppt, ppl = ppl, 
            ppu = ppu, usl = usl, lsl = lsl, target = target))
    }
    invisible(list(lambda = lambda, cp = cp, cpk = cpk, cpl = cpl, cpu = cpu, ppt = ppt, ppl = ppl, 
        ppu = ppu, usl = usl, lsl = lsl, target = target))
}
...
.pcr = function(x, distribution = "normal", lsl, usl, target, boxcox = FALSE, lambda = c(-5, 5), main, 
    ...
    if (!is.null(target)) 
        text(target, max(ylim), "TARGET", pos = 1, col = cex.col, cex = cex.text)
    title(main = main, outer = TRUE)
    return(list(lambda = lambda, cp = cp, cpk = cpk, cpl = cpl, cpu = cpu, ppt = ppt, ppl = ppl, ppu = ppu, 
