...
qweibull3 <- function(p, shape, scale, threshold, ...) {
    ...
    if (missing(scale)) 
        scale = 1
    myfun = function(x, p) pweibull3(q = x, threshold = threshold, scale = scale, shape = shape) - 
        p
    temp = function(p) {
        return(uniroot(f = myfun, lower = threshold, upper = threshold + 1e+07, p = p, ...)$root)
    }
...
dlnorm3 <- function(x, meanlog, sdlog, threshold) {
    ...
    temp = function(x) {
        if (x > threshold) 
            return((1/(sqrt(2 * pi) * sdlog * (x - threshold))) * exp(-(((log((x - threshold)) - meanlog)^2)/(2 * 
                (sdlog)^2))))
        else return(0)
    }
    return(unlist(lapply(x, temp)))
...
qlnorm3 <- function(p, meanlog, sdlog, threshold, ...) {
    ...
    if (missing(sdlog)) 
        sdlog = 1
    myfun = function(x, p) plnorm3(q = x, meanlog = meanlog, sdlog = sdlog, threshold = threshold) - 
        p
    temp = function(p) {
        return(uniroot(f = myfun, lower = threshold, upper = threshold + 1e+07, p = p, ...)$root)
    }
...
qgamma3 <- function(p, shape, scale, threshold, ...) {
    ...
    if (missing(scale)) 
        scale = 1
    myfun = function(x, p) pgamma3(q = x, threshold = threshold, scale = scale, shape = shape) - p
    temp = function(p) {
        return(uniroot(f = myfun, lower = threshold, upper = threshold + 1e+07, p = p, ...)$root)
    }
    return(unlist(lapply(p, temp)))
